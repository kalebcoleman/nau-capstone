...
# Import general constants from utils.py
from utils import ACCENT, APP_BG, COURT_BG, COURT_LINE, MUTED_TEXT, PANEL_BG, PLOT_WIDTH, TEXT_COLOR

REPO_ROOT = Path(__file__).resolve().parents[2]
NHL_DATA_DIR = REPO_ROOT / "nhl" / "data" / "app_data"
...
def sample_nhl_shots_with_far(
    ...
    # Calculate approx distance from center of goal (x=89, y=0)
    # Rink coordinates usually have goal at x=89 or x=-89
    # Just use raw distance for far threshold filtering.
    shots["shot_distance_ft"] = ((shots["xCord"].abs() - 89) ** 2 + shots["yCord"] ** 2) ** 0.5
...
def add_rink_traces(fig, line_color="#FFFFFF"):
    """Add basic NHL rink traces (approximate) to a plotly figure"""
    # Outer boards (200x85)
    outer = dict(
        type="rect",
    ...
    # Blue lines (approx 25 ft from center)
    blue1 = dict(
    ...
    # Goal lines (89 ft from center)
    goal1 = dict(
        type="line", x0=-89, y0=-42.5, x1=-89, y1=42.5,
...
def plot_nhl_scatter_plotly(shots: pd.DataFrame) -> go.Figure:
    ...
    fig = px.scatter(
        ...
        opacity=0.9,
        width=PLOT_WIDTH,
        height=320, # Aspect ratio width/height approx 200/85
    )
    fig.update_traces(
        marker=dict(size=7),
    ...
    # Add shapes using figure layout
    add_rink_traces(fig, line_color=COURT_LINE)
    
    fig.update_xaxes(range=[-100, 100], showgrid=False, zeroline=False, showticklabels=False, title_text="")
    fig.update_yaxes(range=[-42.5, 42.5], showgrid=False, zeroline=False, showticklabels=False, scaleanchor="x", scaleratio=1, title_text="")
    fig.update_layout(
        autosize=False,
...
def compute_nhl_summary(df: pd.DataFrame) -> dict:
    ...
    if "shotType" in df.columns:
        type_counts = df.groupby("shotType")["goal"].agg(shots="size", goals="sum").reset_index()
