...
def sample_shots_with_far(
    ...
    shots["shot_distance_ft"] = (
        (shots["LOC_X"] ** 2 + shots["LOC_Y"] ** 2) ** 0.5
    ) / 10.0
...
def compute_player_sdi_for_season(
    ...
    df = shots_df[shots_df["season"].astype(str) == str(season)].copy()
    if "season_type" in df.columns:
        df = df[df["season_type"].astype(str) == str(season_type)]
    ...
    df["shot_distance_feet"] = pd.to_numeric(
        df["SHOT_DISTANCE"], errors="coerce"
    ).fillna(0)
    df["sdi_distance"] = df["shot_distance_feet"].clip(0, 35) / 35.0

    # 2. Shot clock pressure (less time = harder)
    df["seconds_in_period"] = pd.to_numeric(
        df["MINUTES_REMAINING"], errors="coerce"
    ).fillna(0) * 60 + pd.to_numeric(df["SECONDS_REMAINING"], errors="coerce").fillna(0)
    ...
    # Get usage % from exported usage CSV
    usage_df = load_usage_data().copy()
    if usage_df.empty:
        usage_df = pd.DataFrame(columns=["PLAYER_ID", "usage_pct"])
    else:
        usage_df["PLAYER_ID"] = usage_df.get(
            "personId", usage_df.get("PLAYER_ID")
        ).astype(str)
        usage_df["usage_pct"] = pd.to_numeric(
            usage_df.get("usagePercentage"), errors="coerce"
        )
        usage_df = (
            usage_df.groupby("PLAYER_ID", as_index=False)["usage_pct"].mean().fillna(0)
        )
    ...
    player_sdi["usage_pct"] = player_sdi["usage_pct"].fillna(
        0
...
def plot_sdi_scatter(
    ...
    elite_players = filtered[
        (filtered["residual"] >= 0.07) & (filtered["usage_pct"] >= 0.25)
    ]
...
def main():
    ...
    with st.sidebar:
        ...
        season_default_type = (
            "regular" if "regular" in season_types else season_types[0]
        )
        ...
        season_type = st.selectbox(
            "Season Type", season_types, key="season_type_select"
        )
    ...
    with tab_shots:
        ...
        with col_plot:
            ...
            if isinstance(selection, dict):
                selected_points = selection.get("points", []) or selection.get(
                    "selection", {}
                ).get("points", [])
            else:
        ...
        if sdi_df.empty:
            ...
            with sdi_plot_col:
                ...
                if isinstance(sdi_selection, dict):
                    sdi_selected_points = sdi_selection.get(
                        "points", []
            ...
            with sdi_card_col:
                ...
                if selected_player_name:
                    ...
                    if not player_row.empty:
                        ...
                        if st.button(
                            "📍 View in Shot Map", key="sdi_jump_btn", type="primary"
                        ):
