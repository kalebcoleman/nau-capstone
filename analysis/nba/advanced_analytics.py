...
def load_usage_from_csv(season, season_type):
    ...
    # Filter by season (already 2025-26 in filename)
    if df.empty:
...
def load_assist_pct_from_csv(season, season_type):
    ...
    assist = (
        df.groupby("player_id").apply(weighted_assist).reset_index(name="assist_pct")
    )
    return assist
...
def aggregate_team_residuals(df):
    ...
    zone_stats = (
        ...
        .agg(
            {"xP_prob": "mean", "SHOT_MADE_FLAG": "mean", "residual": ["mean", "count"]}
        )
        .reset_index()
    )
...
def plot_sdi_vs_xfg(player_sdi, output_path):
    ...
    scatter = ax.scatter(
        ...
        cmap="RdYlGn",
        vmin=-0.10,
        vmax=0.10,  # Symmetric colorscale around 0
        edgecolors="black",
        linewidths=0.5,
    )
