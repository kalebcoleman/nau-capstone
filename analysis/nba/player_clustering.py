"""Player clustering and archetype analysis for NBA shot data."""
...
POSITION_CONFIG = {
    "guard_assist_quantile": 0.50,
    "guard_assist_threshold": None,
...
def load_usage_from_csv(season, season_type):
    """Load minutes-weighted usage% from CSV."""
    del season, season_type
    csv_path = DATA_DIR / "player_box_usage_2025-26.csv"
    if not csv_path.exists():
        print(f"Warning: Usage CSV not found at {csv_path}; usage% will be missing.")
    ...
    df["weighted_usage"] = df["usagePercentage"] * df["minutes_num"]
    summary = (
        df.groupby("player_id", as_index=False)
        .agg(
            weighted_sum=("weighted_usage", "sum"),
            minutes_sum=("minutes_num", "sum"),
            usage_mean=("usagePercentage", "mean"),
        )
        .copy()
    )
    summary["usage_pct"] = summary["usage_mean"]
    has_minutes = summary["minutes_sum"] > 0
    summary.loc[has_minutes, "usage_pct"] = (
        summary.loc[has_minutes, "weighted_sum"]
        / summary.loc[has_minutes, "minutes_sum"]
    )
    return summary[["player_id", "usage_pct"]]


def load_position_from_csv(season, season_type):
    """Load minutes-weighted position (G/F/C) from CSV."""
    del season, season_type
    csv_path = DATA_DIR / "player_box_traditional_2025-26.csv"
    if not csv_path.exists():
        print(
            f"Warning: Position CSV not found at {csv_path}; positions will be missing."
        )
...
def load_assist_pct_from_csv(season, season_type):
    """Load minutes-weighted assist% from CSV."""
    del season, season_type
    csv_path = DATA_DIR / "player_box_advanced_2025-26.csv"
    if not csv_path.exists():
        print(f"Warning: Assist CSV not found at {csv_path}; assist% will be missing.")
    ...
    df["weighted_assist"] = df["assistPercentage"] * df["minutes_num"]
    summary = (
        df.groupby("player_id", as_index=False)
        .agg(
            weighted_sum=("weighted_assist", "sum"),
            minutes_sum=("minutes_num", "sum"),
            assist_mean=("assistPercentage", "mean"),
        )
        .copy()
    )
    summary["assist_pct"] = summary["assist_mean"]
    has_minutes = summary["minutes_sum"] > 0
    summary.loc[has_minutes, "assist_pct"] = (
        summary.loc[has_minutes, "weighted_sum"]
        / summary.loc[has_minutes, "minutes_sum"]
    )
    return summary[["player_id", "assist_pct"]]
...
def derive_positions(features, position_df=None, assist_df=None, config=None):
    ...
    if position_df is not None and not position_df.empty:
        ...
        features = features.merge(
            position_df, left_on="PLAYER_ID", right_on="player_id", how="left"
        )
    ...
    if assist_df is not None and not assist_df.empty:
        ...
        features["PLAYER_ID"] = features["PLAYER_ID"].astype(str)
        features = features.merge(
            assist_df, left_on="PLAYER_ID", right_on="player_id", how="left"
        )
        features = features.drop(columns=["player_id"], errors="ignore")
    else:
    ...
    if use_assist and assist_threshold is None:
        ...
        if guard_pool.notna().any():
            assist_threshold = guard_pool.dropna().quantile(
                config.get("guard_assist_quantile", 0.5)
            )
        else:
            all_pool = features["assist_pct"]
            if all_pool.notna().any():
                assist_threshold = all_pool.dropna().quantile(
                    config.get("guard_assist_quantile", 0.5)
                )
            else:
    ...
    if raw is not None:
        ...
        if (
            use_assist
            and assist_threshold is not None
            and not np.isnan(assist_threshold)
        ):
        ...
        pf_mask = (pct_rim >= pf_rim_threshold) | (
            avg_distance <= pf_distance_threshold
        )
    ...
    if missing_raw.any():
        ...
        if (
            ...
            derived.loc[
                missing_raw
                & guard_fallback
                & (features["assist_pct"] >= assist_threshold)
            ] = "PG"
            derived.loc[
                missing_raw
                & guard_fallback
            ...
            guard_dist = config.get("guard_distance_threshold", 14.0)
            guard_3pt = config.get("guard_3pt_threshold", 0.33)
            sg_mask = guard_fallback & (
                (pct_3pt >= guard_3pt) | (avg_distance >= guard_dist)
            )
...
def build_player_features(df, usage_df=None, min_shots=MIN_SHOTS):
    ...
    if "PLAYER_ID" not in df.columns:
        raise ValueError(
            "shots data missing PLAYER_ID; re-run expected_points_analysis.py"
        )
    ...
    volume["attempts_per_game"] = volume["total_attempts"] / volume[
        "games_played"
    ].replace(0, np.nan)
    volume["attempts_per_game"] = volume["attempts_per_game"].fillna(0)

    zone_pcts = (
        df.groupby(["PLAYER_ID", "SHOT_ZONE_BASIC"]).size().unstack(fill_value=0)
    )
    zone_pcts = zone_pcts.div(zone_pcts.sum(axis=1), axis=0)
    zone_pcts.columns = [
        f"pct_{col.replace(' ', '_').lower()}" for col in zone_pcts.columns
    ]
    ...
    distance_stats = (
        df.groupby("PLAYER_ID")["shot_distance_feet"]
        .agg(distance_std="std", distance_iqr=_iqr)
        .reset_index()
    )
    distance_entropy = (
        df.groupby("PLAYER_ID")["shot_distance_feet"]
        .apply(_distance_entropy)
    ...
    features["pct_corner_3"] = features.get("pct_left_corner_3", 0) + features.get(
        "pct_right_corner_3", 0
    )
    ...
    if usage_df is not None and not usage_df.empty:
        ...
        features["PLAYER_ID"] = features["PLAYER_ID"].astype(str)
        features = features.merge(
            usage_df, left_on="PLAYER_ID", right_on="player_id", how="left"
        )
        features = features.drop(columns=["player_id"], errors="ignore")
...
def cluster_players(features, min_k=2, max_k=6):
    ...
    if CLUSTER_FEATURES:
        ...
        exclude_cols = {"player", "PLAYER_ID", "total_attempts", "games_played", "role"}
        feature_cols = [
            c
    ...
    for role in roles:
        ...
        if n_role < 2:
            ...
            selected_meta[role] = {"k": 1, "selection_reason": "single_player_role"}
            continue
        ...
        for k in candidate_ks:
            gmm = GaussianMixture(
                n_components=k, n_init=5, reg_covar=1e-6, random_state=42
            )
            gmm.fit(X_role)
            bic = gmm.bic(X_role)
        ...
        valid_results = [
            r for r in candidate_results if r["min_cluster_size"] >= MIN_CLUSTER_SIZE
        ]
        if valid_results:
        ...
        features.loc[role_mask, "cluster_id"] = [f"{role}-{label}" for label in labels]
...
def label_clusters(features):
    """Assign descriptive labels to clusters based on shot profile."""
    cluster_labels = {}
    ...
    for cluster_id in features["cluster_id"].dropna().unique():
        ...
        usage_val = None
        if (
            "usage_pct" in cluster_data.columns
            and not cluster_data["usage_pct"].isna().all()
        ):
        ...
        elif sdi_median is not None and avg_sdi > sdi_median:
        ...
        cluster_labels[cluster_id] = f"{role} - {base_label}{usage_suffix}"
...
def select_representative_players(features):
    ...
    for archetype, group in features.groupby("archetype"):
        ...
        if volume_col:
            group = group.copy()
            group[volume_col] = pd.to_numeric(
                group[volume_col], errors="coerce"
            ).fillna(0)
...
def plot_position_scatter(features, output_path):
    ...
    positions = features.get(
        "position_derived", pd.Series(["Unknown"] * len(features))
    ).fillna("Unknown")
...
def plot_player_correlation(features, output_path, season_label=None):
    ...
    for i in range(len(labels)):
        for j in range(len(labels)):
            ...
            ax.text(
                j, i, f"{value:.2f}", ha="center", va="center", color=color, fontsize=8
            )
...
def _cluster_output_columns(use_positions):
    output_cols = [
        "player",
        "PLAYER_ID",
        "role",
    ...
    if use_positions:
        output_cols = (
            output_cols[:3]
            + ["position_raw", "position_derived", "position_source", "assist_pct"]
            + output_cols[3:]
        )
    return output_cols
...
def run_player_clustering(
    ...
    """Run clustering phase and persist outputs."""
    ...
    print("=" * 60)
    print(
        "\nDISCLAIMER: Archetypes describe shot location and difficulty patterns only."
    )
    print(
    ...
    plot_player_correlation(
        features,
        Path(figures_dir) / "player_metric_correlations.png",
        season_label=season_label,
    )

    features, _gmm_models, _scaler, bic_df = cluster_players(features, min_k=2, max_k=6)
    ...
    plot_cluster_scatter(features, Path(figures_dir) / "player_archetypes_scatter.png")
    if use_positions:
        plot_position_scatter(
            features, Path(figures_dir) / "player_positions_scatter.png"
        )

    if bic_df is not None and not bic_df.empty:
        bic_df.to_csv(Path(data_dir) / "gmm_bic_by_role.csv", index=False)
        print(f"Saved: {Path(data_dir) / 'gmm_bic_by_role.csv'}")
        plot_gmm_bic(bic_df, Path(figures_dir) / "gmm_bic_by_role.png")

    output_cols = [
        c for c in _cluster_output_columns(use_positions) if c in features.columns
    ]
    features[output_cols].to_csv(Path(data_dir) / "player_clusters.csv", index=False)
    print(f"Saved: {Path(data_dir) / 'player_clusters.csv'}")

    return features, cluster_labels, bic_df
