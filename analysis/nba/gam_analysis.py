...
TRAINING_DATA_PATH = DATA_DIR / "nba_shots_training.csv.gz"
...
def load_shot_data_from_db(
    seasons, season_type="regular", cap_distance=False, exclude_backcourt=False
):
    """Load shot data from CSV for training."""
...
def plot_distance_effect(gam, output_dir, target_df_all=None, obs_max=None):
    ...
    if obs_max is None:
        if target_df_all is not None and "shot_distance_feet" in target_df_all.columns:
            dist_series = pd.to_numeric(
                target_df_all["shot_distance_feet"], errors="coerce"
            )
    ...
    ax.set_ylabel(
        "Marginal log-odds contribution (additive term; 0 = baseline)", fontsize=11
    )
    ax.set_title(
        "GAM Distance Effect on Shot Success (Marginal Log-Odds; Relative to Baseline)",
        fontsize=13,
    )
...
def plot_angle_effect(gam, output_dir):
    ...
    conf = gam.partial_dependence(term=2, X=XX, width=0.95)[1]

    ax.plot(XX[:, 3], pdp, "b-", linewidth=2, label="GAM smooth")
    ax.fill_between(
        XX[:, 3], conf[:, 0], conf[:, 1], alpha=0.2, color="blue", label="95% CI"
    )
    ...
    ax.set_title(
        "Effect of Shot Angle on Make Probability\n(0 = straight on, ±1.57 = baseline)",
        fontsize=14,
    )
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def plot_clock_effect(gam, output_dir):
    ...
    conf = gam.partial_dependence(term=3, X=XX, width=0.95)[1]

    ax.plot(XX[:, 4], pdp, "b-", linewidth=2, label="GAM smooth")
    ax.fill_between(
        XX[:, 4], conf[:, 0], conf[:, 1], alpha=0.2, color="blue", label="95% CI"
    )
    ...
    ax.set_ylabel("Log Odds Contribution", fontsize=12)
    ax.set_title(
        "Effect of Game Clock on Make Probability\n(End-of-period pressure)",
        fontsize=14,
    )
    ax.legend()
...
def plot_period_effect(gam, output_dir):
    ...
    conf = gam.partial_dependence(term=4, X=XX, width=0.95)[1]

    ax.plot(XX[:, 5], pdp, "b-", linewidth=2, label="GAM smooth")
    ax.fill_between(
        XX[:, 5], conf[:, 0], conf[:, 1], alpha=0.2, color="blue", label="95% CI"
    )
    ...
    ax.set_ylabel("Log Odds Contribution", fontsize=12)
    ax.set_title(
        "Effect of Game Period on Make Probability\n(Fatigue and pressure effects)",
        fontsize=14,
    )
...
def plot_shot_type_effects(df, output_dir):
    ...
    ax.set_ylabel("Log Odds Contribution", fontsize=12)
    ax.set_title(
        "Shot Type Effects (Exclusive Categories)\nShot-type-only GAM; baseline = Other",
        fontsize=14,
    )
...
def plot_spatial_tensor(gam, output_dir, df, min_count=25):
    ...
    mesh = ax.pcolormesh(
        XX, YY, Z, cmap="RdYlGn", shading="auto", alpha=0.85, vmin=vmin, vmax=vmax
    )
    ...
    plt.title(
        "GAM Predicted Shot Probability by Location\n(Green = easier, Red = harder; court coords in 0.1 ft)",
        fontsize=16,
    )
    ...
    mesh = ax.pcolormesh(
        XX, YY, effect, cmap="coolwarm", shading="auto", alpha=0.85, vmin=-lim, vmax=lim
    )
    setup_shot_chart_axes(ax)
    cbar = plt.colorbar(mesh, ax=ax)
    cbar.set_label("Log Odds (Spatial Effect)", fontsize=11)
    plt.title(
        "Spatial Effect on Shot Probability\n(GAM Tensor Smooth; court coords in 0.1 ft)",
        fontsize=16,
    )
...
if __name__ == "__main__":
    ...
    training_seasons = get_rolling_window_seasons(
        TARGET_SEASON, window_size=ROLLING_WINDOW
    )
    ...
    train_df = load_shot_data_from_db(
        training_seasons, cap_distance=False, exclude_backcourt=False
    )
    ...
    print(
        f"Model trained on: {len(X_train):,} shots ({training_seasons[0]} to {training_seasons[-1]})"
    )
    print(
        f"Evaluated on:     {len(X_target):,} shots ({TARGET_SEASON}) - TRUE OUT-OF-SAMPLE"
    )
    print("=" * 60)
