...
SAMPLE_DATA_PATH = DATA_DIR / "nba_shots_2025-26.csv.gz"
...
def load_shots(season, season_type="playoffs"):
    """Load shots from CSV sample data."""
    season_type = normalize_season_type(season_type)
    # Read from sample CSV (full columns)
    df = pd.read_csv(SAMPLE_DATA_PATH)
    # Filter by season (CSV already has regular season data)
    df = df[df["season"] == season]
    if df.empty:
        print(f"no rows for season={season} season_type={season_type}")
    return df
...
def generate_shot_chart(
    ...
    if df.empty:
        print(
            f"No shot rows available for {season} ({season_type}); skipping chart render."
        )
        return False
...
if __name__ == "__main__":
    ...
    if not SAMPLE_DATA_PATH.exists():
        raise FileNotFoundError(
            f"Sample data not found at {SAMPLE_DATA_PATH}. Please ensure nba_shots_2025-26.csv.gz exists."
        )
    ...
    rendered_dark = generate_shot_chart(
        ...
        output_filename=str(
            FIGURES_DIR / f"shot_density_hexbin_{current_season}_dark.png"
        ),
    ...
    rendered_light = generate_shot_chart(
        ...
        output_filename=str(
            FIGURES_DIR / f"shot_density_hexbin_{current_season}_light.png"
        ),
        title_text="Shot Density Heatmap Analysis",
        subtitle_text="2025-2026 NBA Regular Season",
        author_name_text="Kaleb Coleman",
